        ...
        {loading ? <div role="status" className={`${panel} p-6`}><p className="text-sm text-muted-foreground">Loading your repository insights…</p><div className="mt-4 h-48 animate-pulse rounded-lg bg-neutral-100" /></div> : <>
          ...
          <div className="grid items-start gap-4 lg:grid-cols-2">
            <section className={panel}>
              ...
              <div className="max-h-80 overflow-auto divide-y divide-border">{attention.map(repo => <Link key={repo.id} to={`/repos/${repo.id}`} state={backState} className="flex items-center justify-between gap-3 p-4 hover:bg-accent/50"><div className="min-w-0"><p className="truncate text-sm font-medium">{repo.name}</p><p className="mt-1 text-xs text-muted-foreground">Last commit · {lastCommit(repo)}</p></div><HealthBadge status={repo.health_status} className="shrink-0" /></Link>)}</div>
            ...
            <section className={panel}>
              ...
              <div className="max-h-80 overflow-auto divide-y divide-border">{reminders.map(repo => <Link key={repo.id} to={`/repos/${repo.id}`} state={backState} className="flex items-center justify-between gap-3 p-4 hover:bg-accent/50"><span className="truncate text-sm">{repo.name}</span><span className="shrink-0 rounded-full bg-violet-50 px-2 py-1 text-xs text-violet-700">{repo.active_reminder_count} reminders</span></Link>)}</div>
