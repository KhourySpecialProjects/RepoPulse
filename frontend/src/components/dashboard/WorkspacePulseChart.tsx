    ...
    <section aria-label="Workspace pulse" className={cn('relative flex min-w-0 flex-col overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm', className)}>
      <div className="relative flex items-center justify-between gap-2 px-5 pt-4">
        ...
        <div className="flex gap-1 rounded-lg border border-slate-200 bg-slate-50 p-0.5" aria-label="Activity period">
          {RANGES.map(range => <button key={range.days} type="button" onClick={() => setDays(range.days)} aria-pressed={days === range.days} className={cn('rounded-md px-2 py-1 text-[11px] font-medium transition-colors focus-visible:ring-2 focus-visible:ring-indigo-500', days === range.days ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-200/70')}>{range.label}</button>)}
        </div>
      </div>
      {failed ? <div role="status" className="flex flex-1 flex-col items-center justify-center gap-3 p-6 text-sm text-slate-500"><p>Commit activity is unavailable.</p><button onClick={onRetry} className="rounded-lg border border-slate-200 px-3 py-2 text-xs text-indigo-600 hover:bg-slate-50">Retry activity</button></div> : <>
        ...
        <div className="relative min-h-24 flex-1 px-2 pt-3">
          {loading ? <div aria-label="Loading commit activity" className="m-2 h-24 animate-pulse rounded-xl bg-slate-100" /> : <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={points} margin={{ top: 4, right: 12, left: 0, bottom: 0 }}>
              <defs><linearGradient id={gradientId} x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#6366f1" stopOpacity={0.35} /><stop offset="100%" stopColor="#6366f1" stopOpacity={0} /></linearGradient></defs>
              ...
              <Tooltip content={({ active, payload, label }) => active && payload?.length ? <div className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs shadow-lg"><p className="text-slate-500">{shortDate(String(label))}</p><p className="mt-1 font-semibold text-indigo-600">{payload[0].value} commits</p></div> : null} />
              <Area type="monotone" dataKey="count" stroke="#4f46e5" strokeWidth={2.5} fill={`url(#${gradientId})`} animationDuration={600} />
            </AreaChart>
          </ResponsiveContainer>}
        </div>
        <div className="relative flex flex-wrap items-center justify-between gap-2 border-t border-slate-100 px-5 py-3 text-[10px] text-slate-500">
          ...
          {!loading && peak?.count > 0 && <span className="inline-flex items-center gap-1"><ArrowUpRight className="h-3 w-3 text-indigo-500" />peak {shortDate(peak.date)} ({peak.count})</span>}
