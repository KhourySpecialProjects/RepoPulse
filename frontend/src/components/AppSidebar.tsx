...
export function AppSidebar() {
  ...
  // Collapsed hides the sidebar entirely; the only affordance left is a floating
  // arrow pinned to the same horizontal line as the collapse arrow it replaces
  // (h-14 header = 56px tall, so top-3 + h-8 centres both at 28px).
  if (collapsed) {
    return (
      <button
        type="button"
        ...
        className="fixed left-3 top-3 z-40 flex h-8 w-8 items-center justify-center rounded-md text-slate-500 hover:text-slate-900 hover:bg-slate-200/70 transition-colors"
      >
