...
describe('AppSidebar collapse — expand arrow does not overlap page content', () => {
  it('renders the expand arrow on its own opaque surface', async () => {
    renderSidebar()
    await waitFor(() => expect(screen.getByText('CS 101 Fall 2025')).toBeInTheDocument())

    fireEvent.click(collapseBtn())

    // Its own box: opaque background, border and shadow so it reads as a
    // distinct control rather than blending into whatever sits beneath it
  ...
  it('reserves a gutter wide enough to clear the arrow', () => {
    // Arrow is left-3 (12px) and h-8/w-8 (32px), so it ends at 44px.
    // The gutter must exceed that or page content slides underneath it.
    expect(COLLAPSED_GUTTER).toBeGreaterThan(44)
  })

  it('offsets main content by the gutter when collapsed, not by zero', async () => {
  ...
  it('offsets main content by the full sidebar width when expanded', async () => {
